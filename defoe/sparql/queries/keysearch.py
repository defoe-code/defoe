...
def do_query(df, config_file=None, logger=None, context=None):
    ...
    target_sentences=config["num_target"]
    lexicon_start= 0
    ...
    if start_year and end_year:
        ...
        newdf=fdf.filter(fdf.definition.isNotNull()).filter(fdf.year >= start_year).select(fdf.year, fdf.definition)
    elif end_year:
        newdf=fdf.filter(fdf.definition.isNotNull()).filter(fdf.year <= end_year).select(fdf.year, fdf.definition)
    else:
    ...
    if target_sentences: 
        target_articles = preprocess_articles.filter(
            lambda year_page: any( target_s in year_page[1] for target_s in target_sentences))
