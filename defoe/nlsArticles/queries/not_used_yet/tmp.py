...
def do_query(archives, config_file=None, logger=None, context=None):
    ...
    documents = archives.flatMap(
        lambda archive: [( document.document_type, document.model, document) for document in list(archive)])
    ...
    pages_clean = documents.flatMap(
        lambda year_document: [(filter_terms_page(page)) for page in year_document[2]])
